...
struct AShooterCharacter : UObject
{
	...
	UAresInventory* get_all_spray_models()
	{
			UObject* Function = UObject::StaticFindObject(nullptr, nullptr, L"ShooterGame.InventoryManager.GetAllSprayModels", false);
			if (!Function)
			{
				return 0;
			}
	...
	void apply_skin(AAresEquippable* GetCurrentWeapon, UObject* skinasset, UObject* chromaasset, int32_t level, UObject* charmdata , int32_t buddielevel)
	{
		UObject* Function = UObject::StaticFindObject(nullptr, nullptr, L"ShooterGame. ContentLibrary.ApplySkin", false);
		struct {
			AAresEquippable* GetCurrentWeapon; UObject* skinasset;
			UObject* chromaasset;
			int32_t level;
			UObject* charmdata;
			int32_t buddielevel1;
		}Args;
		Args.skinasset = skinasset;
		Args.chromaasset = chromaasset;
		Args.level = level;
		Args.charmdata = charmdata;
		Args.buddielevel1 = buddielevel;
		Function->ProcessEvent(static_class(), Function, &Args);
	}
	void clear_weapon_components(AAresEquippable* GetCurrentWeapon)
	{
		UObject* Function = UObject::StaticFindObject(nullptr, nullptr, L"ShooterGame. ContentLibrary.ClearleaponComponents", false);
		struct {
			AAresEquippable* GetCurrentWeapon;
		}Args;
		Function->ProcessEvent(static_class(), Function, &Args);
...
struct APlayerController : UObject
{
	...
	{  
		if (!ProjectWorldToScreen)
			ProjectWorldToScreen = (uintptr_t)GetModuleHandleA(0) + 0x5BDBC20;
...
namespace ShooterGameBlueprints
{
	...
	APlayerController* get_inventory_manager(UObject* WorldContextObject)
	{
		static UObject* function = nullptr;
		if (!function)
			function = UObject::StaticFindObject(nullptr, nullptr, L"ShooterGame.InventoryManager.GetInventoryManager", false);
